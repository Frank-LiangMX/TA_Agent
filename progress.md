...
| 版本 | 日期 | 关键改动 |
|------|------|---------|
| v0.27 | 05-28 | 输入框重构（卡片式工具栏+附件+模型切换+思考模式）、LLM调用日志、隐藏bug修复、审核队列分页修复、置信度推断修复、中断体验优化 |
| v0.26 | 05-21 | 中心服务器框架（FastAPI + SQLite + REST API + 用量管理页面）、客户端双模式设计、MCP 化指南文档 |
| v0.25 | 05-20 | Electron 桌面应用框架（开发/打包模式、启动逻辑优化、窗口闪退修复） |
