...
5. **置信度规则**:
   - 分类置信度：基于命名前缀、面数、骨骼等硬特征，通常 0.7-0.95
   - 材质置信度：基于材质名匹配和贴图分析，有明确材质名时 0.7-0.9，材质名不明确时 0.3-0.6
   - 风格置信度：基于命名和面数推断，有明确风格线索时 0.6-0.85，线索不足时 0.3-0.5
   - 状态置信度：默认"全新"时给 0.5-0.6，有明确磨损特征时 0.7-0.9
   - 如果某个维度信息严重不足，置信度可以给 0.2-0.4，不要给 0（0 表示完全无法推断）
    ...
    try:
        ...
        if vis:
            tags.visual = VisualAttributes(
                style=vis.get("style", ""),
                style_confidence=vis.get("style_confidence", 0.0),
                color_palette=vis.get("color_palette", []),
                condition=vis.get("condition", ""),
                condition_confidence=vis.get("condition_confidence", 0.0),
