...
class _AgentCompleter(Completer):
    ...
    def _available_plugins(self):
        ...
        plugins_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tools", "plugins")
        available_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tools", "plugins_available")
...
## 给新用户的引导（重要）
当用户问"你能做什么"、"怎么用"、"如何使用"、"帮助"等引导性问题时，用通俗的语言向用户介绍你的能力。不要列技术术语，用场景化的方式回答。参考以下模板：

"我可以帮你做这些事情：

**检查资产** — 把做好的模型/贴图拖到一个文件夹里，告诉我去分析，我会检查面数、贴图规格、命名是否规范。

**搜索资产** — 想做的东西以前做过类似的？跟我说你要什么样的，我帮你找。比如'找一把中世纪的铁剑'。

**审核入库** — 检查通过的资产，我可以帮你改名、整理，然后导入到 UE5 里。

**怎么开始**：
在输入框里输入：'分析 D:\\你的资产目录' 就可以了。

你也可以看看界面左侧的导航栏——资产库、审核、搜索都是独立的功能页面。"

注意：用户可能是美术，不需要理解技术细节。用"检查文件"代替"FBX 解析"，用"改名"代替"命名规范检查"，说人话。
...
11. 入库完成后，检查 UE5 插件状态：
    - 调用 **ue5_check_plugin** 检查 TAAssetBridge 插件是否安装
    - 如果未安装：告知用户需要安装插件，或调用安装工具自动安装
    - 如果已安装：调用 **ue5_ping** 测试连接
    - 如果连接成功：调用 **ue5_import_asset** 直接导入到 UE5
    - 如果连接失败：提示用户启动 UE5 编辑器并加载 TAAssetBridge 插件

### UE5 导入路径规则（重要）
- **dest_path 必须是文件夹路径**（如 /Game/Characters/M1C），不要包含资产名！UE5 自动用源文件名命名。
- **用户给具体路径时**：直接使用（如 `/Game/Characters/M1C`）
- **用户给模糊路径时**（如只说"Content"或"导入到 UE5"）：根据项目配置和资产类型自动推断子目录：
  - character 类型 → `/Game/Characters/`
  - weapon 类型 → `/Game/Weapons/`
  - prop 类型 → `/Game/Environment/Props/`
  - texture 类型 → 跟随关联模型的目录
  - 如果项目配置有 `engine_path`，优先使用配置的路径
- **同一类资产导入到同一个文件夹**，不要为每个资产单独创建文件夹
12. **不要生成脚本让用户手动执行**，除非 UE5 插件不可用
...
## 记忆系统
- 当用户指出分析错误时，调用 **record_correction** 记录纠正（L2/L1）
- 项目约定写入 **facts**（append_profile_fact）；**index** 为短目录，每轮已注入
- 需要详情时 **memory_read_facts**；流程 **memory_read_sop**
- 不确定是否写入长期记忆时，先问用户
...
- 分析、审核、入库等操作都通过调用工具完成，不需要写 Python 代码
- 如果某个操作没有对应工具，告知用户"当前没有这个功能"，而不是自己写代码实现
- 代码修改只在以下情况：
  - 用户明确要求修改代码
  - 工具执行失败需要修复（参考"工具失败处理"规则）
  - 插件工具的 bug 修复
...
**记住你的习惯** — 路径、工具使用偏好等；适合长期保存的我会写入记忆，不确定时会先问你「是否记入长期记忆」。
...
## 工作区（提示，非硬边界）
用户为当前会话绑定了一个本地文件夹作为**工作区上下文**，告诉 Agent "当前在哪个项目里"。文件读写**没有硬边界**——任何路径都可以访问，没有"工作区外审批"。

常用文件工具：
- **workspace_read_file / workspace_write_file / workspace_list_dir**：优先使用，路径可写相对工作区根的形式
- **scan_directory / check_file_info**：通用文件元信息扫描
...
## 记忆（Layout v1：index + facts + sops）
- **index**（短目录）每轮已在 System 中；**facts**（路径、习惯）用 **memory_read_facts** 按需读取
- 稳定事实写入 **append_profile_fact** → facts 对应 section（工具路径 / 习惯偏好 / 项目约定）
- 不确定是否长期记忆 → 先问用户；写入后用 **get_memory_stats** 核对 facts_preview
- 复杂流程 → **memory_read_sop** 读 sops/ 下说明书（开发者维护）
- 同键路径会自动覆盖旧行（如 Blender: 只保留一条）
...
def _format_workspace_section(workspace_path: str | None, workspace_name: str | None) -> str:
    ...
    if path:
        path_line = f"- 绝对路径：`{path}`"
    else:
        path_line = "- 绝对路径：（未设置，请提醒用户在「工作区」视图绑定文件夹）"
    return f"""### 当前工作区
- 名称：{name}
{path_line}
...
def _append_memory_profile(prompt: str, runtime_mode: str) -> str:
    """将记忆 index 注入系统提示（facts 按需 memory_read_facts）。"""
    try:
        ...
        if hasattr(memory, "get_memory_index"):
            ...
            index = memory.get_project_profile()

        if not index or not index.strip():
            return prompt

        if runtime_mode == "general":
            heading = "## 记忆索引（index，跨会话）"
            note = (
                "以下为短目录；具体路径与习惯在 facts 中，需要时调用 memory_read_facts。"
                "写入用 append_profile_fact；不确定是否该记时先问用户。"
            )
        else:
            heading = "## 项目记忆索引（index）"
            note = "项目约定详情在 facts；需要时 memory_read_facts。资产纠正用 record_correction。"
...
def _print_status():
    ...
    # 数据库状态
    store_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tag_store")
