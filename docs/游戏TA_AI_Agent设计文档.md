...
> 版本：v0.23 | 创建日期：2026-05-10 | 更新日期：2026-05-18
> 作者：liangmingxuan
> 状态：开发中
...
| 文档 | 路径 | 说明 |
|------|------|------|
| 前端设计文档 | `docs/DESIGN_FRONTEND.md` | 前端架构、组件、协议、设计规范 |
| 流水线设计 | `docs/PIPELINE_DESIGN.md` | 流水线系统详细设计 |
| 工作流程 | `docs/工作流程文档.md` | SVN 集成、团队推广策略 |
| 代码审查 | `docs/CODE_REVIEW_AND_SUGGESTIONS.md` | 外部 agent 代码审查建议 |
| 开发规范 | `CLAUDE.md` | 代码组织、工具开发规范 |
...
| 3. 项目级管理平台 | 🔧 进行中 | 后端完成，前端核心功能完成（会话/资产/审核/搜索），流水线可视化开发中 |
...
| 21 | 流水线系统 | P1 | 4.5 天 | 全栈 | 核心流程可视化 + 自定义阶段 + 执行记录 | 后端✅ 前端开发中 |
...
| 版本 | 日期 | 关键改动 |
|------|------|---------|
| v0.23 | 05-18 | 流水线系统后端、审核维度按类型区分、命名自定义规则、预览图自动渲染、文件过滤模式、类型推断兜底、取消机制、详情面板配置化、骨骼/UV 数据修复、设计文档整理 |
...
# 工具注册
```python
# 工具注册示例（实际实现见 tools/registry.py）
SCHEMA = {
    "type": "function",
    "function": {
        "name": "check_mesh_info",
        "description": "检查模型面数、材质等信息",
        "parameters": { ... }
    }
}

def check_mesh_info(file_path: str) -> dict:
    """工具实现"""
    ...

# 注册到 TOOL_FUNCTIONS
TOOL_FUNCTIONS = {
    "check_mesh_info": check_mesh_info,
    ...
}
```
...
| 3. 项目级管理平台 | 项目团队资产管理 | 资产数据库 + Web 看板 + 多团队分组 + 插件扩展 + 多会话管理 | 进行中（核心功能完成） |
