...
| # | 任务 | 优先级 | 工作量 | 说明 |
|---|------|--------|--------|------|
| 1 | 详情面板配置化 | P1 | 0.5 天 | 字段配置驱动，新增字段只改配置不改组件 |
| 2 | 分析报告导出 | P1 | 0.5 天 | 汇总 + 问题清单 + 导出 |
| 3 | 入库向导 | P2 | 1.5 天 | 分步流程 |
| 4 | 3D 预览 | P3 | 1 天 | Blender 渲染预览图（后端已实现，前端已加按钮） |
...
├── DESIGN.md                      # 前端设计文档（本文件）
├── ARCHITECTURE.md                # 后端协议文档
├── MULTI_USER_ARCHITECTURE.md     # 多用户部署架构
├── CHAT_NAVIGATION.md             # 会话导航设计
│
...
| 功能 | 说明 | 涉及文件 | 状态 |
|------|------|---------|------|
| 会话选择器 | MainPanel 头部：会话标题按钮 + Popover 弹出会话列表 | `components/session/SessionSelector.tsx` | ❌ 待开发 |
| 会话列表 | Popover 内：搜索 + 置顶 + 日期分组 + 归档折叠 | `components/session/SessionPopover.tsx` | ❌ 待开发 |
| 会话操作 | 新建、切换、删除、置顶、归档 | `services/sessions.ts` | ❌ 待开发 |
| WebSocket sessionId | 连接时带 sessionId，切换会话时自动重连 | `services/websocket.ts` | ❌ 待开发 |
| 历史恢复 | 切换会话时从后端加载历史消息 | `MainPanel.tsx` | ❌ 待开发 |
